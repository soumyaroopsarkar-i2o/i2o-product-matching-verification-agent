"""Run Rainforest Collections for all Loreal UPCs (type=product, gtin=...).
...
RAINFOREST_API_KEY = os.environ["RAINFOREST_API_KEY"]
...
CACHE_FILE = BASE_DIR / "loreal_upc_collection_rainforest_cache.json"
RESULTS_DIR = BASE_DIR / "loreal_upc_results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
OUT_JSONL = RESULTS_DIR / "loreal_upc_amazon.jsonl"

COLLECTION_NAME = "Loreal_UPC_Amazon_Product"

logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
log = logging.getLogger("loreal_upc_rf")
...
def create_collection(name: str) -> str | None:
    ...
    r = http.post(f"{RAINFOREST_BASE_URL}/collections", params={"api_key": RAINFOREST_API_KEY}, json=body)
    r.raise_for_status()
...
def add_requests(cid: str, reqs: list[dict]) -> bool:
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        r = http.put(
            f"{RAINFOREST_BASE_URL}/collections/{cid}",
...
def start_collection(cid: str) -> bool:
    r = http.get(f"{RAINFOREST_BASE_URL}/collections/{cid}/start", params={"api_key": RAINFOREST_API_KEY})
    if r.status_code >= 400:
...
def poll_until_complete(cid: str):
    ...
    while True:
        try:
            r = http.get(f"{RAINFOREST_BASE_URL}/collections/{cid}", params={"api_key": RAINFOREST_API_KEY})
            r.raise_for_status()
            coll = r.json().get("collection", {})
            status = coll.get("status")
            count = coll.get("results_count", 0)
            total = coll.get("requests_total_count", "?")
            log.info(f"  status={status}  results={count}  requests_total={total}")
...
def download_results(cid: str, out_file: Path, cache_entry: dict) -> bool:
    if not cache_entry.get("download_links"):
        r = http.get(
            f"{RAINFOREST_BASE_URL}/collections/{cid}/results/1",
            params={"api_key": RAINFOREST_API_KEY, "format": "json"},
        )
        r.raise_for_status()
...
def main():
    ...
    requests_payload = [
        {"type": "product", "amazon_domain": AMAZON_DOMAIN, "gtin": u} for u in upcs
    ]
