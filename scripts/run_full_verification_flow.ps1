...
if (-not $InputWorkbook) {
    $InputWorkbook = Join-Path $repoRoot "loreal_wmt_attributes\loreal_wmt_attributes\lorealpi_product_verification_input.xlsx"
}
if (-not $OutputRoot) {
...
$prepareStage1 = Join-Path $repoRoot "scripts\prepare_claude_p_batches.py"
$runStage1 = Join-Path $repoRoot "scripts\run_claude_p_batches.ps1"
$prepareStage2A = Join-Path $repoRoot "scripts\prepare_price_pair_batches.py"
$parseStage2A = Join-Path $repoRoot "scripts\parse_price_pair_response.py"
$mergeStage2A = Join-Path $repoRoot "scripts\merge_price_pair_results.py"
$prepareStage2B = Join-Path $repoRoot "scripts\prepare_marketplace_outlier_batches.py"
$parseStage2B = Join-Path $repoRoot "scripts\parse_marketplace_outlier_response.py"
$mergeStage2B = Join-Path $repoRoot "scripts\merge_marketplace_outlier_results.py"
