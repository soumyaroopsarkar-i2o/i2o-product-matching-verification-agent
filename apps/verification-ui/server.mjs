...
const workflowRoot = path.join(repoRoot, 'workflows', 'verification');
...
async function prepareRun(inputWorkbook, runId) {
  const prepareScript = path.join(workflowRoot, 'prepare_claude_p_batches.py');
...
async function runClaudeBatches(runDir, model, effort, onProgress) {
  const parseScript = path.join(workflowRoot, 'parse_claude_p_response.py');
...
async function mergeRun(runDir) {
  const mergeScript = path.join(workflowRoot, 'merge_claude_p_results.py');
  await runProcess(pythonExe, [mergeScript, '--run-dir', runDir]);
}

async function preparePricePairRun(inputWorkbook, outputRoot, runId) {
  const prepareScript = path.join(workflowRoot, 'prepare_price_pair_batches.py');
  await runProcess(pythonExe, [
    prepareScript,
    '--input-workbook',
    inputWorkbook,
...
async function prepareMarketplaceOutlierRun(inputWorkbook, outputRoot, runId) {
  const prepareScript = path.join(workflowRoot, 'prepare_marketplace_outlier_batches.py');
  await runProcess(pythonExe, [
    prepareScript,
    '--input-workbook',
    inputWorkbook,
...
async function mergePricePairRun(runDir, outputWorkbook) {
  const mergeScript = path.join(workflowRoot, 'merge_price_pair_results.py');
...
async function mergeMarketplaceOutlierRun(runDir, outputWorkbook) {
  const mergeScript = path.join(workflowRoot, 'merge_marketplace_outlier_results.py');
  await runProcess(pythonExe, [mergeScript, '--run-dir', runDir, '--output-workbook', outputWorkbook]);
}
...
async function executeVerificationJob(jobId) {
  ...
  try {
    ...
    await runAgentBatches(stage2ARunDir, path.join(workflowRoot, 'parse_price_pair_response.py'), job.model, job.effort, (progressPatch) => {
      const total = progressPatch.totalBatches ?? jobs.get(jobId)?.totalBatches ?? 0;
      const completed = progressPatch.completedBatches ?? jobs.get(jobId)?.completedBatches ?? 0;
    ...
    await runAgentBatches(stage2BRunDir, path.join(workflowRoot, 'parse_marketplace_outlier_response.py'), job.model, job.effort, (progressPatch) => {
      const total = progressPatch.totalBatches ?? jobs.get(jobId)?.totalBatches ?? 0;
      const completed = progressPatch.completedBatches ?? jobs.get(jobId)?.completedBatches ?? 0;
